...
import { markAsSeenSchema } from "@shared/types";
...
import { getEpisode, saveAsSeen } from "./db";
...
app
  ...
  .get("/cache/check", async (c) => {
    ...
    try {
      ...
      // Check cache
      try {
        const cachedResult = await c.env.WHISPER_JOBS.get(cacheKey);
        if (cachedResult) {
          return c.json({
            cached: true,
  ...
  .get("/itunes/lookup", async (c) => {
    ...
    try {
      const episode = url ? await getEpisode(c.env, url) : undefined;
      if (episode) {
        const ret: iTunesResponse = {
          resultCount: 1,
          results: [episode as unknown as iTunesResult],
  ...
  .post("/itunes/mark-as-seen", async (c) => {
    let ret: MarkAsSeenResponse;

    const result = markAsSeenSchema.safeParse(await c.req.json());
    if (!result.success) {
      ret = {
        success: false,
        error: JSON.stringify(result.error.flatten().fieldErrors),
      };
      return c.json(ret, 400);
    }
    const { episodeUrl, iTunesResult } = result.data;
    try {
      await saveAsSeen(c.env, episodeUrl, iTunesResult);
      ret = { success: true };
  ...
  .post("/transcribe/complete", async (c) => {
    try {
      ...
      try {
        await c.env.WHISPER_JOBS.put(cacheKey, JSON.stringify(result));
